...
class Text2MelDataLoader(DataLoader):
    def __init__(self, text2mel_dataset, batch_size, mode='train', num_workers=4):
        if mode == 'train':
...
class SSRNDataLoader(DataLoader):
    def __init__(self, ssrn_dataset, batch_size, mode='train', num_workers=4):
        if mode == 'train':
...
def collate_fn(batch):
    ...
    for key in keys:
        collated_batch[key] = int(default_collate(collated_batch[key])).long()
